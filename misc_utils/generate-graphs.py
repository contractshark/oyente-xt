...
def gen_contract_hist():
    ...
    print "Max: %d, Min: %d, Mean: %d, Median: %d" % (
        max(oplengths),
        min(oplengths),
        np.mean(oplengths),
        np.median(oplengths),
...
def gen_txnesting_hist(tfile):
    nesting = [
        (transaction["txid"], len(transaction["transactions"]))
...
def gen_txworth_hist(tfile):
    txworth = [
        (
            transaction["txid"],
            process_worth_str(transaction["transactions"][0]["worth"]),
        )
        for transaction in tqdm(tfile)
    ]
    txworth = sorted(txworth, key=lambda k: k[1], reverse=True)
    txvalues = [element[1] for element in txworth]
    print "Transaction value - min:%f, max:%f, mean:%f, median:%f" % (
        np.min(txvalues),
        np.max(txvalues),
        np.mean(txvalues),
        np.median(txvalues),
...
def gen_contract_tx(tfile):
    ...
    print "Min Balance: %d, Max Balance: %d, Sum: %f, Mean Balance: %f, Median Balance: %f" % (
        np.min(balances),
        np.max(balances),
        np.sum(balances),
        np.mean(balances),
...
def check_unique():
    ...
    for cfile in tqdm(cfiles):
        if not cfile.endswith(".json"):
            continue
        if cfile == "contract_balance.json":
            continue
...
def get_all():
    ...
    old_prob = all_problematic[:]

    for c in reentrancy:
        all_problematic.append(c)

    print "%d transaction race, %d timestamp, %d in cstack, %d in reentrancy." % (
        len(t_race),
        len(ts_depend),
        len(cstack),
        len(reentrancy),
    ...
    print "CallStack - %d, Transaction Race - %d, Timestamp Dependence - %d, Reentrancy - %d" % (
        len(new_cstack),
        len(new_t_race),
        len(new_ts_depend),
        len(new_reentrancy),
...
def get_source_stats():
    ...
    for contract in tqdm(contracts):
        if len(get_contract_code(contract)[1]) > 0:
            with_source += 1
