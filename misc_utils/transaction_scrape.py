...
def download_txs(dirn):
    ...
    os.system(
        "curl 'https://etherscan.io/txsInternal?valid=false&p=[1-%d]'' -o %stxs#1.html"
        % (max_p, dirn)
    )
...
def download_txs_all(dirn):
    ...
    print "Max pages = %d" % (int(max_p))
    os.system(
        "curl 'https://etherscan.io/txsInternal?p=[1-%d]' -o %stxs#1.html"
        % (int(max_p), dirn)
    )
    print "Download complete."
    os.system("rm tmp.html")
...
def load_txfile(fn):
    ...
    return [
        run_re_file(re_str_from, fn)[0],
        run_re_file(re_str_to, fn)[0],
        run_re_file(re_str_gasgiven, fn)[0],
        run_re_file(re_str_gasused, fn)[0],
        run_re_file(re_str_cumgas, fn)[0],
        process_value(run_re_file(re_str_value, fn)[0]),
    ]
