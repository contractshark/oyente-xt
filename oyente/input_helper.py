...
class InputHelper:
    ...
    def _extract_bin_obj(self, com: CryticCompile):
        return [
            (
                com.contracts_filenames[name].absolute + ":" + name,
                com.bytecode_runtime(name),
            )
            for name in com.contracts_names
            if com.bytecode_runtime(name)
        ]
    ...
    def _compile_solidity(self):
        try:
            ...
            com = CryticCompile(
                self.source, solc_remaps=self.remap, solc_args=" ".join(options)
            )
