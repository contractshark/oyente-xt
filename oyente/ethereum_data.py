...
class EthereumData:
    ...
    def getBalance(self, address):
        try:
            apiEndPoint = (
                ...
                % (self.apiDomain, address, self.apikey)
            )
    ...
    def getCode(self, address):
        try:
            apiEndPoint = (
                "%s?module=proxy&action=eth_getCode&address=%s&tag=latest&apikey=%s"
                % (self.apiDomain, address, self.apikey)
            )
            r = requests.get(apiEndPoint)
    ...
    def getStorageAt(self, position):
        try:
            ...
            if position[-1] == "L":
                position = position[:-1]
            apiEndPoint = (
                "%s?module=proxy&action=eth_getStorageAt&address=%s&position=%s&tag=latest&apikey=%s"
                % (self.apiDomain, self.contract_addr, position, self.apikey)
            )
            r = requests.get(apiEndPoint)
            result = r.json()["result"]
        except Exception as e:
            if str(e) != "timeout":
                log.exception(
                    "Error at: contract address: %s, position: %s"
                    % (self.contract_addr, position)
                )
            raise
